...
function renderTodos() {
  ...
  todos.forEach((todo, index) => {
    ...
    // 근데 얜 type이 text가 아니라 checkbox니까
    ...
    // 요소들을 li 변수에 추가해야 한다. 왜 ? li 내부에 체크박스 / 스팬 / 버튼
    li.append(checkbox);
    li.append(span);
    li.append(deleteBtn);

    // 그리고 이 li를 <ul> 태그에 추가해야한다.
    ...
    deleteBtn.addEventListener('click', () => {
      // 삭제할 항목의 텍스트를 이용해서 배열 내에서 정확한 인덱스를 찾아낼거다.
      // 보통은 index로 내용을 확인하지만 계속 삭제를 하면 index 넘버가 바뀐다.
      // 그래서 input에 입력했던 내용을 근거로 index를 역으로 찾아낼거다.
      const itemText = span.textContent; // 얠 쓰는 이유는 .trim()쓰는 바람에 공백을 날렸기 때문이다.
      ...
      if (itemIndex !== -1) { // 일치하는 인덱스가 없으면 -1
        todos.splice(itemIndex, 1); // itemIndex에 해당하는 거 element 하나 삭제
        // li.remove();
        saveTodos();
      }
    });
...
function addTodo() {
  const todoText = todoInput.value.trim();    // trim() 공백 제거 method 였다.
  if (todoText === '') {
    alert('내용을 입력하세요: ');
    return;
  }
...
// 새로고침을 했을 때 renderTodos() 가 호출 되야함.
